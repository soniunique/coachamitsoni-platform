...
function AdminAssessment({ courseId }: { courseId: string }) {
  ...
  return <section className="learn-card mt-6 p-6">
    ...
    <div className="mt-5 grid gap-5 md:grid-cols-2">
      ...
      <div><label className="mb-2 block text-sm font-medium">Maximum attempts</label><input type="number" min="0" value={maxAttempts} onChange={e => setMaxAttempts(Number(e.target.value))} className="w-full rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white"/><p className="mt-2 text-xs text-slate-500">0 = unlimited attempts. Passed assessments can be retaken until the maximum is reached.</p></div>
    </div>
    ...
    <div className="mt-7 border-t border-white/10 pt-6"><div className="flex flex-wrap items-center justify-between gap-3"><div><h3 className="font-semibold">Question bank</h3><p className="text-xs text-slate-500">Write questions against the learning outcomes. Add explanations for high-quality formative feedback.</p></div><button type="button" onClick={addQuestion} className="learn-secondary-button"><Plus size={15}/>Add question</button></div>
      ...
      <div className="mt-6 flex justify-end border-t border-white/10 pt-5"><button type="button" onClick={() => void save()} disabled={saving} className="learn-primary-button">{saving ? <Loader2 size={16} className="animate-spin"/> : <Save size={16}/>} {saving ? "Saving..." : "Save assessment"}</button></div>
    </div>
  </section>;
}
...
function StudentAssessment({ slug, isAdmin }: { slug: string; isAdmin: boolean }) {
  ...
  const attemptsRemaining = assessment.max_attempts == null || !latest ? true : latest.attempt_number < assessment.max_attempts;
  const finalAttemptUsed = Boolean(latest && assessment.max_attempts != null && latest.attempt_number >= assessment.max_attempts);
  const canStart = completion?.can_start !== false && attemptsRemaining;
  const attemptsLabel = assessment.max_attempts ? `${assessment.max_attempts} maximum` : "Unlimited";
  const startLabel = latest?.passed ? "Retake Assessment" : "Start Assessment";
  ...
  return <section className="learn-card mt-8 p-6" aria-labelledby="assessment-heading">
    ...
    {completion && assessment.require_completion && !completion.can_start && !finalAttemptUsed && <div className="mt-5 rounded-xl border border-amber-500/30 bg-amber-500/10 p-4 text-sm text-amber-200"><strong>Assessment locked.</strong> Complete all course lessons first. Progress: {completion.lessons_completed} / {completion.lessons_total} lessons ({completion.percentage}%).</div>}
    ...
    {finalAttemptUsed && latest && !session && !result && <div className={`mt-5 rounded-2xl border p-5 ${latest.passed ? "border-emerald-500/30 bg-emerald-500/10" : "border-red-500/30 bg-red-500/10"}`} role="status"><div className="flex items-center gap-2 text-lg font-bold">{latest.passed ? <CheckCircle2 size={20}/> : <CircleAlert size={20}/>} {latest.passed ? "Assessment Passed — No attempts remaining" : "Maximum Attempts Reached"}</div><p className="mt-2 text-sm text-slate-300">{latest.passed ? `You scored ${latest.score}%. You passed on your final allowed attempt (${latest.attempt_number} of ${assessment.max_attempts}). No further attempts are available.` : `You scored ${latest.score}%. You did not reach the passing score of ${assessment.passing_percentage}%. You have used all ${assessment.max_attempts} allowed attempts. No further attempts are available.`}</p></div>}

    {result && <div className={`mt-5 rounded-2xl border p-5 ${result.passed ? "border-emerald-500/30 bg-emerald-500/10" : "border-amber-500/30 bg-amber-500/10"}`}><div className="flex items-center gap-2 text-lg font-bold">{result.passed ? <CheckCircle2 size={20}/> : <CircleAlert size={20}/>} {result.passed ? "Assessment passed" : "Assessment not passed"}</div><p className="mt-1 text-sm text-slate-300">You scored <strong>{result.score}%</strong>. Passing score is {result.passing_percentage}%. Attempt {result.attempt_number}.</p>{result.timed_out && <p className="mt-2 text-sm text-amber-200">{result.feedback}</p>}{result.review?.length ? <div className="mt-4 space-y-3">{result.review.map((item, i) => <div key={item.question_id} className="rounded-xl border border-white/10 bg-black/10 p-3 text-sm"><div className="font-medium">Question {i + 1}: {item.correct ? "Correct" : "Incorrect"}</div>{item.correct_option && <div className="mt-1 text-slate-400">Correct answer: {item.correct_option}</div>}{item.explanation && <div className="mt-1 text-slate-300">{item.explanation}</div>}</div>)}</div> : null}{result.passed && !finalAttemptUsed && hasAttemptsRemainingForResult(result, assessment) && <button type="button" onClick={() => setResult(null)} className="learn-secondary-button mt-4">Retake Assessment</button>}{!result.passed && canStart && <button type="button" onClick={() => setResult(null)} className="learn-secondary-button mt-4">Try again</button>}{result.attempt_number >= (assessment.max_attempts || Number.MAX_SAFE_INTEGER) && <div className="mt-4 text-sm font-semibold text-slate-300">{result.passed ? "Assessment Passed — No attempts remaining" : "Maximum Attempts Reached"}</div>}</div>}

    {!session && !result && !finalAttemptUsed && <div className="mt-6 rounded-2xl border border-white/10 bg-white/[.03] p-5"><div className="grid gap-4 sm:grid-cols-3"><div><div className="text-xs text-slate-500">Questions</div><div className="mt-1 font-semibold">{assessment.question_count || "All in bank"}</div></div><div><div className="text-xs text-slate-500">Time limit</div><div className="mt-1 font-semibold">{assessment.time_limit_minutes ? `${assessment.time_limit_minutes} minutes` : "Unlimited"}</div></div><div><div className="text-xs text-slate-500">Randomization</div><div className="mt-1 font-semibold">{assessment.randomize_questions || assessment.randomize_options ? "Enabled" : "Off"}</div></div></div>{assessment.integrity_ack_required && <label className="mt-5 flex items-start gap-3 rounded-xl border border-white/10 bg-white/5 p-4 text-sm"><input type="checkbox" checked={acknowledged} onChange={e => setAcknowledged(e.target.checked)} className="mt-1"/><span><strong>I understand the assessment rules.</strong><span className="block text-xs leading-5 text-slate-500">I will complete this assessment independently and understand that my submitted attempt is final.</span></span></label>}<button type="button" onClick={() => void start()} disabled={!canStart || starting} className="learn-primary-button mt-5">{starting ? <Loader2 size={16} className="animate-spin"/> : <ShieldCheck size={16}/>} {starting ? "Starting..." : startLabel}</button></div>}
...
function hasAttemptsRemainingForResult(result: Result, assessment: Assessment) {
  return assessment.max_attempts == null || result.attempt_number < assessment.max_attempts;
}
