...
function AdminAssessment({ courseId }: { courseId: string }) {
  ...
  async function save() {
    ...
    for (const [i, q] of questions.entries()) {
      if (!q.prompt.trim()) { setError(`Question ${i + 1} needs a question prompt.`); setSaving(false); return; }
      if (q.options.length < 2 || q.options.some(o => !o.text.trim())) { setError(`Question ${i + 1} needs at least two completed options.`); setSaving(false); return; }
      if (!q.options.some(o => o.id === q.correct_option)) { setError(`Question ${i + 1} has an invalid correct answer.`); setSaving(false); return; }
    }
    ...
    if (questions.length) {
      const rows = questions.map((q, i) => ({ assessment_id: saved.id, prompt: q.prompt.trim(), options: q.options.map(o => ({ id: o.id, text: o.text.trim() })), correct_option: q.correct_option, points: Math.max(1, Number(q.points) || 1), sort_order: i }));
  ...
  const updateOption = (qi: number, oi: number, text: string) => setQuestions(prev => prev.map((q, i) => i === qi ? { ...q, options: q.options.map((o, j) => j === oi ? { ...o, text } : o) } : q));
  ...
  return <section className="learn-card mt-6 p-6">
    ...
    <div className="mt-7 border-t border-white/10 pt-6"><div className="flex items-center justify-between"><div><h3 className="font-semibold">Questions</h3><p className="text-xs text-slate-500">Multiple-choice questions are graded automatically.</p></div><button type="button" onClick={() => setQuestions(prev => [...prev, { ...blankQuestion(), sort_order: prev.length }])} className="learn-secondary-button"><Plus size={15}/>Add question</button></div>
      <div className="mt-4 space-y-4">{questions.map((q, qi) => <div key={q.id || `new-${qi}`} className="rounded-2xl border border-white/10 bg-white/[.03] p-5"><div className="flex items-start justify-between gap-3"><div className="text-xs font-semibold uppercase tracking-[.15em] text-slate-500">Question {qi + 1}</div><button type="button" onClick={() => setQuestions(prev => prev.filter((_, i) => i !== qi))} className="learn-icon-button text-red-300" aria-label="Delete question"><Trash2 size={16}/></button></div><textarea rows={3} value={q.prompt} onChange={e => updateQuestion(qi, { prompt: e.target.value })} className="mt-3 w-full rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white" placeholder="What is the correct answer?"/><div className="mt-4 grid gap-3 md:grid-cols-2">{q.options.map((o, oi) => <div key={o.id} className="flex items-center gap-2"><input type="radio" name={`correct-${qi}`} checked={q.correct_option === o.id} onChange={() => updateQuestion(qi, { correct_option: o.id })}/><input value={o.text} onChange={e => updateOption(qi, oi, e.target.value)} className="min-w-0 flex-1 rounded-xl border border-white/10 bg-white/5 px-3 py-2.5 text-sm text-white" placeholder={`Option ${o.id.toUpperCase()}`}/></div>)}</div><div className="mt-4 w-32"><label className="mb-2 block text-xs text-slate-500">Points</label><input type="number" min="1" value={q.points} onChange={e => updateQuestion(qi, { points: Number(e.target.value) })} className="w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm text-white"/></div></div>)}</div>
    </div>
  </section>;
}
