...
export function LearnShell({ children }: { children: React.ReactNode }) {
  ...
  return <div className="learn-app min-h-screen bg-[#07111f] text-white">
    <aside className={`learn-sidebar ${open ? "is-open" : ""}`}>
      <div className="flex items-center justify-between gap-3 px-5 py-5"><Link to="/learn" className="flex items-center gap-3" onClick={() => setOpen(false)}><img src={logo} alt="AI Agents & Cloud Architecture Coach" className="h-10 w-auto"/><div className="hidden xl:block"><div className="font-display text-sm font-bold">Amit Soni</div><div className="text-[10px] text-slate-400">Learning Hub</div></div></Link><button className="learn-mobile-close lg:hidden" onClick={() => setOpen(false)} aria-label="Close menu"><X size={18}/></button></div>
      <div className="px-3 pt-4"><div className="px-3 pb-2 text-[10px] font-bold uppercase tracking-[.2em] text-slate-500">Learn</div><nav className="space-y-1">
        {nav.slice(0,3).map(({label,to,icon:Icon})=>{const isActive=to==="/learn"?active==="/learn":active.startsWith(to);return <Link key={to} to={to} onClick={()=>setOpen(false)} className={`learn-nav-item ${isActive?"active":""}`}><Icon size={18}/><span>{label}</span></Link>})}
        ...
        {isAdmin && <Link to="/learn/manage/enrolments" onClick={()=>setOpen(false)} className={`learn-nav-item ${active.startsWith("/learn/manage/enrolments")||active.startsWith("/learn/manage/course-students")?"active":""}`}><UserCheck size={18}/><span>Program Access</span></Link>}
