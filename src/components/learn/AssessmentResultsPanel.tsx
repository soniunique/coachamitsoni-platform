    ...
    <section className="learn-card mt-7 p-6" aria-labelledby="student-results-heading">
            ...
            <tbody>
              {attempts.map((attempt) => (
                <tr key={attempt.attempt_id} className="border-b border-white/5">
                  ...
                  <td className="px-3 py-3"><span className={`inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-xs font-medium ${attempt.passed ? "bg-emerald-500/10 text-emerald-300" : "bg-amber-500/10 text-amber-200"}`}>{attempt.passed ? <CheckCircle2 size={13} /> : <XCircle size={13} />}{attempt.passed ? "Passed" : "Failed"}</span></td>
      ...
      {selected && (
        <div className="mt-6 rounded-2xl border border-white/10 bg-white/[.03] p-5">
          <div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="font-semibold">{selected.student_name} — Attempt #{selected.attempt_number}</h3><p className="text-xs text-slate-500">{selected.student_email} · {new Date(selected.submitted_at).toLocaleString()}</p></div><button type="button" onClick={() => setSelected(null)} className="learn-secondary-button">Close</button></div>
          <div className="mt-4 grid gap-3 sm:grid-cols-3"><div className="rounded-xl border border-white/10 p-3"><div className="text-xs text-slate-500">Score</div><div className="mt-1 text-lg font-bold">{selected.score}%</div></div><div className="rounded-xl border border-white/10 p-3"><div className="text-xs text-slate-500">Result</div><div className="mt-1 font-bold">{selected.passed ? "Passed" : "Failed"}</div></div><div className="rounded-xl border border-white/10 p-3"><div className="text-xs text-slate-500">Submitted</div><div className="mt-1 text-sm">{new Date(selected.submitted_at).toLocaleString()}</div></div></div>
          <div className="mt-5"><h4 className="text-sm font-semibold">Answer review</h4>{questions.length ? <div className="mt-3 space-y-3">{questions.map((question, index) => { const answerId = selectedAnswers[question.id]; const correct = answerId === question.correct_option; return <div key={question.id} className="rounded-xl border border-white/10 p-4"><div className="flex items-start justify-between gap-3"><div className="text-sm font-medium">{index + 1}. {question.prompt}</div><span className={`text-xs font-medium ${correct ? "text-emerald-300" : "text-amber-200"}`}>{correct ? "Correct" : "Incorrect"}</span></div><div className="mt-2 text-xs text-slate-400">Submitted answer: <span className="text-slate-200">{optionText(question, answerId)}</span></div><div className="mt-1 text-xs text-slate-400">Correct answer: <span className="text-slate-200">{optionText(question, question.correct_option)}</span></div>{question.explanation && <div className="mt-2 text-xs leading-5 text-slate-400">Explanation: {question.explanation}</div>}</div>; })}</div> : <p className="mt-3 text-sm text-slate-500">Question details are unavailable for this assessment.</p>}</div>
        </div>
      )}
    </section>
  );
