...
export function ProgramPurchaseButton({ programId, programTitle, priceInr }: { programId: string; programTitle: string; priceInr: number }) {
  ...
  useEffect(() => {
    ...
    void supabase.auth.getUser().then(({ data: { user } }) => {
      ...
      // Do not prefill or suggest the site owner's name. The purchaser must enter their own full name.
    });
  }, [open]);
  ...
  async function startPayment() {
    ...
    try {
      ...
      // Close the site's form modal before opening Razorpay. Keeping two modal layers
      // mounted at the same time can cause the checkout overlay to flicker or render behind it.
      setOpen(false);
  ...
  return <>
    ...
    <LearnModal
      ...
      description={success ? "Your payment has been verified and program access is now active." : "Enter your details, then complete the secure Razorpay checkout."}
