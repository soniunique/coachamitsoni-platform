...
function Courses() {
  ...
  useEffect(() => {
    async function loadCourses() {
      ...
      if (programError || courseError) setDbError(programError?.message ?? courseError?.message ?? "Unable to load courses right now.");
      else { setPrograms((programData ?? []) as Program[]); setCourses((courseData ?? []) as Course[]); }
      setLoading(false);
    }
    void loadCourses();
  }, []);
