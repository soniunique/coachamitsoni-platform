...
function Courses() {
  ...
  return <LearnShell><SectionHeader eyebrow="My learning" title="Programs & Courses" description="Continue your learning journey and track your progress"/>
    {loading ? <div className="learn-card flex items-center gap-3 p-6 text-sm text-slate-400"><Loader2 size={18} className="animate-spin"/>Loading your programs...</div> : dbError ? <div className="learn-card p-6 text-sm text-red-300">{dbError}</div> : visiblePrograms.length === 0 ? <div className="learn-card p-10 text-center"><BookOpen className="mx-auto text-cyan-300" size={32}/><h2 className="mt-4 text-lg font-bold">No program access yet</h2><p className="mt-2 text-sm text-slate-400">Your courses will appear here</p></div> : <div className="space-y-10">{visiblePrograms.map(program=>{const programCourses=courses.filter(course=>course.program_id===program.id); const hasAccess=enrolledPrograms.has(program.id); return <section key={program.id}><div className="mb-5"><div className="flex flex-wrap items-start justify-between gap-4"><div className="min-w-0"><div className="text-[11px] font-bold uppercase tracking-[.14em] text-cyan-300">Program</div><h2 className="mt-1 text-2xl font-bold leading-tight">{program.title}</h2>{program.description&&<p className="mt-2 max-w-3xl text-sm leading-6 text-slate-400">{program.description}</p>}</div>{hasAccess&&<span className="learn-status-pill learn-status-registered mt-1 shrink-0"><CheckCircle2 size={13}/>Enrolled</span>}</div>{!hasAccess&&program.payment_enabled&&program.price_inr>0&&<div className="mt-5 flex flex-col gap-4 rounded-xl border border-cyan-400/15 bg-cyan-400/5 p-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-start gap-3"><LockKeyhole size={18} className="mt-0.5 shrink-0 text-cyan-300"/><div><div className="text-sm font-semibold text-white">Unlock this program</div><p className="mt-1 text-xs leading-5 text-slate-400">One purchase gives you access to all {programCourses.length} published course{programCourses.length===1?"":"s"} in this program.</p></div></div><div className="sm:w-64"><ProgramPurchaseButton programId={program.id} programTitle={program.title} priceInr={program.price_inr}/></div></div>}</div><div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">{programCourses.map(course=>{const p=progressFor(course.id); return <article className="learn-card learn-course-card overflow-hidden" key={course.id}><div className="relative h-44 w-full overflow-hidden bg-slate-900/70"><img src={course.thumbnail_url || ""} alt="" className="h-full w-full object-cover" onError={e=>{e.currentTarget.style.display="none";const fallback=e.currentTarget.nextElementSibling;if(fallback instanceof HTMLElement)fallback.classList.remove("hidden");}}/><div className={`absolute inset-0 flex items-center justify-center bg-gradient-to-br from-cyan-500/15 via-violet-500/15 to-orange-500/15 ${course.thumbnail_url?"hidden":""}`}><BookOpen size={40} className="text-cyan-300/70"/></div></div><div className="p-6"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><h3 className="text-[19px] font-bold leading-6">{course.title}</h3><div className="mt-1 text-sm text-slate-500">{program.title}</div></div>{p.percent===100&&<CheckCircle2 size={18} className="mt-0.5 shrink-0 text-emerald-400" aria-label="Completed"/>}</div><p className="mt-3 min-h-12 text-sm leading-6 text-slate-400">{course.description||"Explore this course."}</p><div className="mt-6"><div className="flex items-center justify-between text-sm"><span className="font-medium text-slate-300">Progress</span><span className="font-semibold text-cyan-300">{p.percent}%</span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-white/8"><div className="h-full rounded-full bg-gradient-to-r from-cyan-400 to-violet-500" style={{width:`${p.percent}%`}}/></div><div className="mt-2 text-xs text-slate-500">{p.done} of {p.total} lessons complete</div></div><Link to="/learn/courses/$slug" params={{slug:course.slug}} className="learn-secondary-button mt-6 w-full justify-center">{p.percent>0&&p.percent<100?"Continue course":"View course"} <ArrowUpRight size={15}/></Link></div></article>})}</div></section>})}</div>}
  </LearnShell>;
}
