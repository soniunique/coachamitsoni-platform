...
function contentIcon(type: string | null) {
  ...
  if (type === "pdf" || type === "article") return FileText;
  return Link2;
}
...
function CourseDetail() {
  ...
  async function load() {
    ...
    setExpanded(new Set(loadedModules.map((module) => module.id)));
    ...
    if (user) {
      ...
      const saved = window.localStorage.getItem(`course-rating:${courseData.id}`);
  ...
  async function openLesson(lesson: Lesson | null, markAsComplete = false) {
    ...
    if (!admin) {
      ...
      if (!enrollment) {
        ...
        setSelected(null);
        setUrl(null);
        setError("Course enrolment is required to open lessons.");
        return;
      }
    }
    ...
    if (freshError || !freshLesson) {
      setError("This lesson is no longer available.");
      return;
    }
    setIsAdmin(admin);
    setEnrolled(true);
    setSelected(freshLesson.id);
  ...
  async function saveLessonCompletion(lessonId: string, value: boolean, userId?: string) {
    ...
    else setCompleted((previous) => { const next = new Set(previous); value ? next.add(lessonId) : next.delete(lessonId); return next; });
    setSavingComplete(false);
  }

  async function toggleSelectedCompletion() {
    if (!selectedLesson || !enrolled || isAdmin) return;
    await saveLessonCompletion(selectedLesson.id, !completed.has(selectedLesson.id));
  }

  function rate(value: number) {
    setRating(value);
    if (course) window.localStorage.setItem(`course-rating:${course.id}`, String(value));
  }
  ...
  return <LearnShell>
    ...
    <section className="reference-course-hero mt-4">
      <div className="grid gap-6 md:grid-cols-[250px_1fr] md:items-start">
        ...
        <div>
          ...
          <p className="mt-2">{course.description || "Complete the lessons in this course at your own pace."}</p>
    ...
    {selectedLesson ? <section className="reference-player-shell mt-4">
      <div className="reference-player">
        <div className="reference-player-frame">
        ...
        <div className="reference-player-meta">
          <div className="flex items-start justify-between gap-3"><div><h2 className="font-bold">{selectedLesson.title}</h2>{selectedLesson.description && <p className="mt-1">{selectedLesson.description}</p>}</div>{completed.has(selectedLesson.id) && <span className="inline-flex shrink-0 items-center gap-1 rounded-full bg-emerald-100 px-2 py-1 text-[10px] font-bold text-emerald-700"><Check size={12} />Completed</span>}</div>
      ...
      <aside className="reference-content-panel">
        <div className="reference-content-header"><span>Content</span><span className="text-[10px] font-medium text-slate-500">{doneCount} of {lessons.length}</span></div>
        {modules.map((module, moduleIndex) => {
          const open = expanded.has(module.id);
          const moduleLessons = lessons.filter((lesson) => lesson.module_id === module.id);
          return <div className="reference-content-section" key={module.id}>
            <button type="button" onClick={() => setExpanded((previous) => { const next = new Set(previous); next.has(module.id) ? next.delete(module.id) : next.add(module.id); return next; })}>
              <span>{open ? <ChevronDown size={14} /> : <ChevronRight size={14} />}</span><span>{moduleIndex + 1}. {module.title}</span><span>{moduleStats.get(module.id) || 0}</span>
            </button>
