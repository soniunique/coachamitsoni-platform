...
function AnnouncementsManager() {
  ...
  return <LearnShell>
    ...
    {!items.length ? <div className="learn-card p-8 text-center text-sm text-slate-400"><Megaphone className="mx-auto mb-3 text-cyan-300" size={28} />No announcements yet.</div> : <div className="space-y-4">{items.map(a => <article key={a.id} className="learn-card p-6"><div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between"><div><div className="learn-eyebrow">{audienceLabel(a)}</div><h2 className="mt-2 text-xl font-bold">{a.title}</h2><p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-slate-400">{a.body}</p><div className="mt-3 text-xs text-slate-500">{new Date(a.published_at).toLocaleString()}</div></div><div className="flex shrink-0 items-start gap-2"><button onClick={() => edit(a)} className="learn-secondary-button"><Pencil size={15} />Edit</button><button onClick={() => void remove(a)} className="learn-secondary-button"><Trash2 size={15} />Delete</button></div></div></article>)}</div>}
  </LearnShell>;
}
