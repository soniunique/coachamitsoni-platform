...
function AnnouncementsManager() {
  ...
  function edit(a: Announcement) {
    setDraft({ id: a.id, title: a.title, body: a.body, audienceType: a.audience_type, programId: a.program_id || "" });
    setSuccess(""); setError("");
  }

  function create() {
    setDraft({ title: "", body: "", audienceType: "all", programId: "" });
    setSuccess(""); setError("");
  }

  function setAudience(value: string) {
    if (!draft) return;
    if (value === "all") setDraft({ ...draft, audienceType: "all", programId: "" });
    else setDraft({ ...draft, audienceType: "program", programId: value });
  }
  ...
  async function save(e: React.FormEvent) {
    ...
    const values = {
      ...
      program_id: draft.audienceType === "program" ? draft.programId : null,
      updated_at: new Date().toISOString(),
    };
    const result = draft.id
      ? await supabase.from("announcements").update(values).eq("id", draft.id)
      : await supabase.from("announcements").insert({ ...values, created_by: (await supabase.auth.getUser()).data.user?.id }).select().single();
    if (result.error) { setError(result.error.message); setSaving(false); return; }
    setDraft(null);
    setSuccess(draft.id ? "Announcement updated successfully." : "Announcement published successfully.");
    await load();
    setSaving(false);
  }
  ...
  async function remove(a: Announcement) {
    ...
    const { error: removeError } = await supabase.from("announcements").delete().eq("id", a.id);
    if (removeError) setError(removeError.message);
    else { setSuccess("Announcement deleted successfully."); await load(); }
  }

  function audienceLabel(a: Announcement) {
    if (a.audience_type === "all") return "All students";
    return programs.find(p => p.id === a.program_id)?.title || "Specific program";
  }
  ...
  return <LearnShell>
    ...
    {draft && <form onSubmit={save} className="learn-card mb-6 space-y-5 p-6">
      <div className="flex items-center justify-between"><div className="learn-eyebrow">{draft.id ? "Edit announcement" : "New announcement"}</div><button type="button" onClick={() => setDraft(null)} className="learn-secondary-button"><X size={15} />Cancel</button></div>
      <input value={draft.title} onChange={e => setDraft({ ...draft, title: e.target.value })} placeholder="Title" className="w-full rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white" />
      <textarea value={draft.body} onChange={e => setDraft({ ...draft, body: e.target.value })} rows={5} placeholder="Message" className="w-full rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white" />
      <div className="rounded-2xl border border-cyan-400/20 bg-cyan-400/[0.04] p-4">
        <div className="text-sm font-semibold text-white">Announcement audience</div>
        <p className="mt-1 text-xs leading-5 text-slate-400">Choose <strong className="text-slate-300">All students</strong> for a global announcement, or select a <strong className="text-slate-300">Program</strong> to link it to that program. Program announcements are shown only to students enrolled in that program.</p>
        <label className="mt-4 block text-sm text-slate-300">Program / audience
          <select value={draft.audienceType === "all" ? "all" : draft.programId} onChange={e => setAudience(e.target.value)} className="mt-2 w-full rounded-xl border border-white/10 bg-[#172333] px-4 py-3 text-sm text-white">
            <option value="all">All students</option>
            {programs.map(program => <option key={program.id} value={program.id}>{program.title}</option>)}
          </select>
        </label>
        {draft.audienceType === "program" && draft.programId && <div className="mt-3 text-xs text-cyan-300">✓ Linked to program: {programs.find(p => p.id === draft.programId)?.title || "Selected program"}</div>}
        {!programs.length && <div className="mt-3 text-xs text-amber-300">No programs are available yet. Create a program first if this announcement should be program-specific.</div>}
      </div>
      <div className="flex justify-end"><button disabled={saving} className="learn-primary-button">{saving ? <Loader2 size={16} className="animate-spin" /> : <Save size={16} />} {saving ? "Saving..." : draft.id ? "Save changes" : "Publish announcement"}</button></div>
    </form>}
