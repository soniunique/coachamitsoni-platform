...
function CourseStudentsManager() {
  ...
  return <LearnShell>
    ...
    <div className="learn-card p-6">
      ...
      <tbody>{filteredRoster.map(student=>{const active=student.enrollment_status==='active'; const completed=student.enrollment_status==='completed'; const busy=savingId===student.id; return <tr key={student.id} className="border-b border-white/6 last:border-0"><td className="px-3 py-4"><div className="font-semibold">{student.full_name||"Unnamed student"}</div><div className="mt-1 text-xs text-slate-500">{student.email}</div></td><td className="px-3 py-4">{active?<span className="inline-flex items-center gap-1 rounded-full border border-emerald-400/20 bg-emerald-400/10 px-2.5 py-1 text-xs text-emerald-300"><CheckCircle2 size={13}/>Full course</span>:completed?<span className="inline-flex rounded-full border border-cyan-400/20 bg-cyan-400/10 px-2.5 py-1 text-xs text-cyan-300">Completed</span>:<span className="rounded-full border border-white/10 px-2.5 py-1 text-xs text-slate-400">No access</span>}</td><td className="px-3 py-4 text-slate-400">{active||completed?`${Number(student.progress_percent??0).toFixed(0)}%`:"—"}</td><td className="px-3 py-4 text-right">{active?<button type="button" className="learn-secondary-button" disabled={busy} onClick={()=>void unenrol(student)}>{busy?<Loader2 size={15} className="animate-spin"/>:<UserRoundX size={15}/>}Un-enrol</button>:<button type="button" className="learn-primary-button" disabled={busy} onClick={()=>void setEnrollment(student.id,true)}>{busy?<Loader2 size={15} className="animate-spin"/>:<UserPlus size={15}/>}Enrol full course</button>}</td></tr>})}</tbody></table>{filteredRoster.length===0&&<div className="py-10 text-center text-sm text-slate-500">No student accounts match your search.</div>}</div>
    </div>
  </LearnShell>;
}
