...
import { BookOpen, Layers, Loader2, Pencil, Plus, Save, Trash2, X } from "lucide-react";
...
function CourseManager() {
  ...
  async function load() {
    ...
    setPrograms((programData ?? []) as Program[]); setCourses((courseData ?? []) as Course[]); setLoading(false);
  }
  ...
  function closeForms() { if (saving) return; setProgramFormOpen(false); setCourseFormOpen(false); setEditingProgramId(null); setEditingCourseId(null); setProgramForm(emptyProgram); setCourseForm(emptyCourse); }
  ...
  async function saveProgram(event: React.FormEvent<HTMLFormElement>) {
    ...
    setSuccess(editingProgramId ? "Program updated successfully." : "Program created successfully."); closeForms(); setSaving(false); await load();
  }
  ...
  async function saveCourse(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (result.error) { setError(result.error.message); setSaving(false); return; }
    setSuccess(editingCourseId ? "Course updated successfully." : "Course created successfully."); closeForms(); setSaving(false); await load();
  }
  ...
  return <LearnShell>
    <SectionHeader eyebrow="Admin" title="Programs & Courses" description="Programs contain courses. Student access is granted at the program level and automatically covers every course inside it." action={isAdmin ? <div className="flex gap-2"><button type="button" className="learn-secondary-button" onClick={openCreateProgram}><Plus size={17}/>New program</button><button type="button" className="learn-primary-button" onClick={() => openCreateCourse()} disabled={programs.length === 0}><Plus size={17}/>New course</button></div> : undefined} />
    ...
    {isAdmin && courseFormOpen && <form onSubmit={saveCourse} className="learn-card mb-6 space-y-5 p-6"><div className="flex items-center justify-between"><div><div className="learn-eyebrow">{editingCourseId ? "Edit course" : "New course"}</div><h2 className="mt-1 text-xl font-bold">{editingCourseId ? "Update course" : "Create course inside a program"}</h2></div><button type="button" className="learn-icon-button" onClick={closeForms}><X size={18}/></button></div><div><label className="mb-2 block text-sm font-medium">Program <span className="text-cyan-300">*</span></label><select value={courseForm.program_id} onChange={e=>setCourseForm(c=>({...c,program_id:e.target.value}))} className="w-full rounded-xl border border-white/10 bg-[#111d2d] px-4 py-3 text-sm text-white"><option value="">Select a program</option>{programs.map(program=><option key={program.id} value={program.id}>{program.title}</option>)}</select><p className="mt-2 text-xs text-slate-500">Students assigned to this program will automatically receive access to this course.</p></div><div className="grid gap-5 md:grid-cols-2"><div><label className="mb-2 block text-sm font-medium">Course title</label><input value={courseForm.title} onChange={e=>setCourseForm(c=>({...c,title:e.target.value}))} className="w-full rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white" placeholder="Build Your First AI Agent"/></div><div><label className="mb-2 block text-sm font-medium">Slug</label><input value={courseForm.slug} onChange={e=>setCourseForm(c=>({...c,slug:e.target.value.toLowerCase().replace(/\s+/g,"-")}))} className="w-full rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white" placeholder="build-your-first-ai-agent"/></div></div><div><label className="mb-2 block text-sm font-medium">Description</label><textarea rows={4} value={courseForm.description} onChange={e=>setCourseForm(c=>({...c,description:e.target.value}))} className="w-full resize-none rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white"/></div><div className="grid gap-5 md:grid-cols-2"><div><label className="mb-2 block text-sm font-medium">Thumbnail URL</label><input type="url" value={courseForm.thumbnail_url} onChange={e=>setCourseForm(c=>({...c,thumbnail_url:e.target.value}))} className="w-full rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white"/></div><div><label className="mb-2 block text-sm font-medium">Status</label><select value={courseForm.status} onChange={e=>setCourseForm(c=>({...c,status:e.target.value as Status}))} className="w-full rounded-xl border border-white/10 bg-[#111d2d] px-4 py-3 text-sm text-white"><option value="draft">Draft</option><option value="published">Published</option><option value="archived">Archived</option></select></div></div><div className="flex justify-end gap-3"><button type="button" className="learn-secondary-button" onClick={closeForms}>Cancel</button><button type="submit" className="learn-primary-button" disabled={saving}>{saving?<><Loader2 size={17} className="animate-spin"/>Saving...</>:<><Save size={17}/>Save course</>}</button></div></form>}

    {loading ? <div className="learn-card flex items-center gap-3 p-6 text-slate-400"><Loader2 size={18} className="animate-spin"/>Loading programs and courses...</div> : isAdmin && <div className="space-y-6">
      {programs.length === 0 ? <div className="learn-card p-8 text-center"><BookOpen className="mx-auto text-cyan-300" size={32}/><h2 className="mt-4 text-lg font-bold">No programs yet</h2><p className="mt-2 text-sm text-slate-400">Create a program first, then add courses inside it.</p></div> : programs.map(program => <section key={program.id} className="learn-card overflow-hidden"><div className="border-b border-white/10 p-6"><div className="flex flex-wrap items-start justify-between gap-4"><div><div className="flex items-center gap-3"><h2 className="text-xl font-bold">{program.title}</h2><span className="rounded-full border border-white/10 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wide text-slate-300">{program.status}</span></div><p className="mt-1 text-xs text-slate-500">{program.slug} · {coursesFor(program.id).length} course{coursesFor(program.id).length === 1 ? "" : "s"}</p><p className="mt-3 max-w-2xl text-sm leading-6 text-slate-400">{program.description || "No description added yet."}</p></div><div className="flex gap-2"><button type="button" className="learn-secondary-button" onClick={()=>openEditProgram(program)}><Pencil size={15}/>Edit program</button><button type="button" className="learn-primary-button" onClick={()=>openCreateCourse(program.id)}><Plus size={15}/>Add course</button><button type="button" className="learn-icon-button" onClick={()=>void deleteProgram(program)} aria-label={`Delete ${program.title}`}><Trash2 size={16}/></button></div></div></div><div className="p-6">{coursesFor(program.id).length === 0 ? <p className="text-sm text-slate-500">No courses in this program yet.</p> : <div className="grid gap-4 md:grid-cols-2">{coursesFor(program.id).map(course=><article key={course.id} className="rounded-2xl border border-white/10 bg-white/[0.03] p-5"><div className="flex items-start justify-between gap-3"><div><h3 className="font-semibold">{course.title}</h3><p className="mt-1 text-xs text-slate-500">{course.slug}</p></div><span className="rounded-full border border-white/10 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wide text-slate-300">{course.status}</span></div><p className="mt-3 text-sm leading-6 text-slate-400">{course.description || "No description added yet."}</p><div className="mt-4 flex gap-2"><button type="button" className="learn-secondary-button flex-1" onClick={()=>openEditCourse(course)}><Pencil size={15}/>Edit</button><Link to="/learn/manage/course-content/$courseId" params={{courseId: course.id}} className="learn-secondary-button flex-1"><Layers size={15}/>Content</Link><button type="button" className="learn-secondary-button" onClick={()=>void deleteCourse(course)} aria-label={`Delete ${course.title}`}><Trash2 size={15}/></button></div></article>)}</div>}</div></section>)}
    </div>}
  </LearnShell>;
}
