...
const enrolledDeleteMessage = "This program already has enrollment records. The program and its courses cannot be deleted because learner history must be preserved. Archive or edit them instead.";
...
function CourseManager() {
  ...
  async function load() {
    ...
    const loadedPrograms = (programData ?? []) as Program[];
    setPrograms(loadedPrograms); setCourses(loadedCourses);

    const { data: enrollmentData, error: enrollmentError } = await supabase.from("program_enrollments").select("program_id");
    if (enrollmentError) { setError(enrollmentError.message); setLoading(false); return; }
    setEnrolledProgramIds(new Set((enrollmentData ?? []).map(row => row.program_id)));
  ...
  async function deleteProgram(program: Program) {
    ...
    if (enrolledProgramIds.has(program.id)) { setError(enrolledDeleteMessage); return; }
  ...
  async function saveCourse(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (payload.status === "published") {
      const r = editingCourseId ? readiness[editingCourseId] : undefined;
      const missingModules = !r || r.modules === 0;
      const missingLessons = !r || r.lessons === 0;
      const missingAssessment = Boolean(r?.assessmentRequired && !r.assessmentExists);
      if (missingModules || missingLessons || missingAssessment) {
        const requirements: string[] = [];
        if (missingModules) requirements.push("at least 1 module");
        if (missingLessons) requirements.push("at least 1 lesson");
        if (missingAssessment) requirements.push("the required assessment");
        setError(`Cannot publish this course yet. Minimum required: ${requirements.join(", ")}. Add the missing content, then try publishing again.`);
  ...
  async function deleteCourse(course: Course) {
    if (enrolledProgramIds.has(course.program_id)) { setError(enrolledDeleteMessage); return; }
  ...
  return <LearnShell>
    ...
    {error && !courseFormOpen && <div className="learn-card mb-5 border border-red-500/30 bg-red-500/10 p-4 text-sm text-red-300">{error}</div>}
