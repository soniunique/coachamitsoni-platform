...
function Certificates() {
  ...
  async function printCertificate(certificate: Certificate) {
    ...
    popup.document.write(`<html><head><title>Certificate of Completion - ${safeCourse}</title></head><body><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1120 690"><rect width="1120" height="690" fill="#fffdf8"/><rect x="10" y="10" width="1100" height="670" fill="none" stroke="#062b4f" stroke-width="8"/><text x="560" y="100" text-anchor="middle" font-size="13" font-weight="700" fill="#078da6">CERTIFICATE OF COMPLETION</text><text x="560" y="220" text-anchor="middle" font-size="47" font-weight="700" fill="#102b4e">Certificate of Completion</text><text x="560" y="340" text-anchor="middle" font-size="42" font-style="italic" fill="#18365a">${safeName}</text><text x="560" y="400" text-anchor="middle" font-size="15" fill="#64748b">for successfully completing</text><text x="560" y="445" text-anchor="middle" font-size="30" font-weight="700" fill="#078da6">${safeCourse}</text><text x="425" y="510" text-anchor="middle" font-size="10" font-weight="700" fill="#102b4e">ISSUED</text><text x="425" y="530" text-anchor="middle" font-size="12" fill="#596579">${safeDate}</text><text x="695" y="510" text-anchor="middle" font-size="10" font-weight="700" fill="#102b4e">CERTIFICATE NO.</text><text x="695" y="530" text-anchor="middle" font-size="12" fill="#596579">${safeNumber}</text><text x="560" y="580" text-anchor="middle" font-size="25" font-style="italic" fill="#1d2939">Amit Soni</text></svg><script>window.onload=()=>setTimeout(()=>window.print(),250)</script></body></html>`); popup.document.close(); setPrinting(null);
  }

  return <LearnShell><SectionHeader eyebrow="MY LEARNING" title="Certificates" description="View and print certificates you have earned." /><div className="mx-auto max-w-5xl px-6 pb-12"><Link to="/learn" className="mb-6 inline-flex items-center gap-2 text-sm text-slate-300 hover:text-white"><ArrowLeft size={16}/> Back to learning</Link>{loading?<div className="flex items-center gap-2 text-slate-300"><Loader2 className="animate-spin" size={18}/> Loading certificates…</div>:error?<div className="rounded-2xl border border-rose-400/30 bg-rose-400/10 p-5 text-rose-200">{error}</div>:certificates.length===0?<div className="rounded-2xl border border-white/10 bg-slate-900/50 p-8 text-center"><Award className="mx-auto mb-3 text-slate-400" size={34}/><h2 className="text-lg font-semibold text-white">No certificates yet</h2><p className="mt-2 text-sm text-slate-400">Complete eligible courses to earn certificates.</p></div>:<div className="grid gap-5 md:grid-cols-2">{certificates.map(c=><div key={c.id} className="rounded-2xl border border-white/10 bg-slate-900/60 p-6"><h2 className="text-xl font-semibold text-white">{c.course?.title||"Course"}</h2><p className="mt-2 text-sm text-slate-400">Certificate No. {c.certificate_number}</p><p className="text-sm text-slate-400">Issued {new Date(c.issued_at).toLocaleDateString()}</p><button onClick={()=>void printCertificate(c)} disabled={printing===c.id} className="mt-5 inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-cyan-300 to-violet-400 px-4 py-2 text-sm font-semibold text-slate-950">{printing===c.id?<Loader2 className="animate-spin" size={16}/>:<Printer size={16}/>} Print certificate</button></div>)}</div>}</div></LearnShell>;
}
