...
function Discussions() {
  ...
  return <LearnShell>
    ...
    {!courses.length ? <div className="learn-card p-10 text-center"><MessageSquare className="mx-auto text-cyan-300" size={34}/><h2 className="mt-4 text-lg font-bold">No course discussions yet</h2><p className="mt-2 text-sm text-slate-500">Join a program to access its course community.</p></div> : <>
      ...
      {threads.length === 0 ? <div className="learn-card flex min-h-[360px] items-center justify-center p-8 text-center"><div className="mx-auto max-w-md"><div className="mx-auto flex h-12 w-12 items-center justify-center rounded-2xl border border-cyan-400/20 bg-cyan-400/10 text-cyan-300"><MessageSquare size={24}/></div><h2 className="mt-5 text-xl font-bold">No discussions yet</h2><p className="mt-2 text-sm leading-6 text-slate-400">Start the first discussion for {course?.title || "this course"} and invite learners to ask questions, share ideas, and learn together.</p><button type="button" onClick={() => { setError(""); setShowComposer(true); }} className="learn-primary-button mt-5"><Plus size={16}/>Start the first discussion</button></div></div> : <div className="grid gap-5 lg:grid-cols-[360px_1fr]">
        <aside className="learn-card overflow-hidden"><div className="border-b border-white/10 px-5 py-4 text-xs font-bold uppercase tracking-[.16em] text-slate-500">Topics · {threads.length}</div><div className="divide-y divide-white/5">{threads.map(t => <button key={t.id} type="button" onClick={() => setOpenThread(t.id)} className={`w-full p-5 text-left ${t.id === openThread ? "bg-cyan-400/8" : "hover:bg-white/5"}`}><div className="flex items-start gap-2"><div className="min-w-0 flex-1"><div className="font-semibold leading-5">{t.title}</div><div className="mt-2 text-xs text-slate-500">{t.author?.full_name || "Member"} · {new Date(t.updated_at).toLocaleDateString()}</div></div>{t.pinned && <Pin size={15} className="shrink-0 text-cyan-300"/>}{t.locked && <Lock size={14} className="shrink-0 text-amber-300"/>}</div></button>)}</div></aside>
        <main className="learn-card min-h-[520px] p-6">
          {activeThread ? <div><div className="border-b border-white/10 pb-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="text-2xl font-bold">{activeThread.title}</h2><div className="mt-2 text-xs text-slate-500">{activeThread.author?.full_name || "Member"} · {new Date(activeThread.created_at).toLocaleString()}</div></div>{isAdmin && <div className="flex items-center gap-3"><span className="text-[10px] font-bold uppercase tracking-[.16em] text-slate-500">Moderation</span><div className="flex gap-2"><button type="button" aria-pressed={activeThread.pinned} title={activeThread.pinned ? "Unpin discussion" : "Pin discussion"} onClick={() => void moderate("pinned", !activeThread.pinned)} className="learn-secondary-button min-w-[84px] justify-center"><Pin size={14}/>{activeThread.pinned ? "Unpin" : "Pin"}</button><button type="button" aria-pressed={activeThread.locked} title={activeThread.locked ? "Unlock discussion" : "Lock discussion"} onClick={() => void moderate("locked", !activeThread.locked)} className="learn-secondary-button min-w-[84px] justify-center">{activeThread.locked ? <Unlock size={14}/> : <Lock size={14}/>} {activeThread.locked ? "Unlock" : "Lock"}</button></div></div>}</div><p className="mt-4 whitespace-pre-wrap text-sm leading-7 text-slate-300">{activeThread.body}</p></div><div className="space-y-4 py-5">{(replies[activeThread.id] || []).map(r => <div key={r.id} className="rounded-xl border border-white/8 bg-white/[.03] p-4"><div className="flex items-center gap-2 text-xs"><ShieldCheck size={14} className={r.author?.role === "admin" ? "text-cyan-300" : "text-slate-500"}/><span className="font-semibold text-slate-200">{r.author?.full_name || "Member"}</span>{r.author?.role === "admin" && <span className="text-cyan-300">Admin</span>}<span className="text-slate-600">{new Date(r.created_at).toLocaleString()}</span></div><p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-slate-300">{r.body}</p></div>)}{!(replies[activeThread.id] || []).length && <div className="py-5 text-sm text-slate-500">No replies yet.</div>}</div>{activeThread.locked ? <div className="rounded-xl border border-amber-400/20 bg-amber-400/5 p-4 text-sm text-amber-200">This discussion is locked by an administrator.</div> : <div className="flex gap-2"><textarea value={replyBody} onChange={e => setReplyBody(e.target.value)} className="learn-input min-h-20 resize-y" placeholder="Write a reply…" maxLength={10000}/><button type="button" onClick={() => void createReply()} disabled={saving || !replyBody.trim()} className="learn-primary-button self-end"><Send size={15}/></button></div>}</div> : <div className="flex h-full items-center justify-center text-sm text-slate-500">Select a discussion.</div>}
        </main>
      </div>}
    </>}
  </LearnShell>;
