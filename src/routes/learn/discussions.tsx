...
function Discussions() {
  ...
  return <LearnShell>
    <SectionHeader eyebrow="Community" title="Discussions" description="Ask questions, share ideas and learn together inside each course." action={<button type="button" onClick={() => setShowComposer(v => !v)} disabled={!courseId} className="learn-primary-button"><Plus size={16}/>{showComposer ? "Close" : "New discussion"}</button>} />
    ...
    {!courses.length ? <div className="learn-card p-10 text-center"><MessageSquare className="mx-auto text-cyan-300" size={34}/><h2 className="mt-4 text-lg font-bold">No course discussions yet</h2><p className="mt-2 text-sm text-slate-500">Join a program to access its course community.</p></div> : <>
      ...
      {showComposer && <div className="learn-card mb-5 p-5"><div className="learn-eyebrow">Start a thread</div><input value={newTitle} onChange={e => setNewTitle(e.target.value)} className="learn-input mt-3" placeholder="Discussion title" maxLength={200}/><textarea value={newBody} onChange={e => setNewBody(e.target.value)} className="learn-input mt-3 min-h-32 resize-y" placeholder="What would you like to discuss?" maxLength={10000}/><div className="mt-3 flex justify-end"><button type="button" onClick={() => void createThread()} disabled={saving || !newTitle.trim() || !newBody.trim()} className="learn-primary-button"><Send size={15}/>{saving ? "Posting…" : "Post discussion"}</button></div></div>}
