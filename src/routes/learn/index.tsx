...
import { ArrowRight, BookOpen, CalendarDays, CheckCircle2, Clock3, MessageSquare, Sparkles, Video } from "lucide-react";
...
function LearnHome() {
  ...
  const nextCourse = inProgress[0] ?? courses.find((course) => course.progress === 0) ?? completed[0] ?? null;

  return <LearnShell>
    <SectionHeader eyebrow="Upcoming Workshops" title="Upcoming Workshops" description="Join upcoming live sessions, catch the latest updates and continue your learning." action={<Link to="/learn/workshops" className="learn-see-all">See all <ArrowRight size={14}/></Link>} />
    <div className="learn-feed-layout">
      <div className="learn-feed-primary">
        <section className="mb-6"><div className="mb-3 flex items-end justify-between"><div><div className="learn-eyebrow">Upcoming workshops</div><h2 className="mt-1 text-xl font-bold">Join the next live session</h2></div><Link to="/learn/workshops" className="learn-see-all">See all <ArrowRight size={14}/></Link></div>{workshops.length ? <div className="space-y-3">{workshops.map((workshop) => <article key={workshop.id} className="reference-workshop-card upcoming"><div className="reference-workshop-grid"><div><div className="reference-workshop-status"><Video size={13}/> Upcoming workshop</div><h3 className="mt-2 text-lg font-bold">{workshop.title}</h3><p className="mt-1 text-sm leading-6">{workshop.description || "A live practical session from Coach Amit Soni."}</p><div className="reference-workshop-meta"><span><CalendarDays size={13}/>{dateLabel(workshop.starts_at)}</span><span><Clock3 size={13}/>{timeLabel(workshop.starts_at)}</span><span><Video size={13}/>{workshop.format || "Online"}</span></div></div><div className="reference-workshop-actions"><Link to="/learn/workshops" className="reference-register-button">View details <ArrowRight size={13}/></Link></div></div></article>)}</div> : <div className="learn-card p-5 text-sm">No upcoming workshops are scheduled yet.</div>}</section>
        <section className="mb-6"><div className="mb-3"><div className="learn-eyebrow">Latest from Amit</div><h2 className="mt-1 text-xl font-bold">Feed</h2></div>{posts.length ? <div className="grid gap-3 md:grid-cols-2">{posts.map((post) => <article key={post.id} className="learn-card p-5"><div className="flex items-start gap-3"><div className="learn-avatar">AS</div><div className="min-w-0"><h3 className="font-bold">{post.title || "Learning update"}</h3><p className="mt-2 text-sm leading-6">{post.body}</p><div className="mt-3 text-[10px] text-slate-500">Amit Soni · {new Date(post.created_at).toLocaleDateString()}</div></div></div></article>)}</div> : <div className="learn-card p-5 text-sm">No feed updates have been published yet.</div>}</section>
      </div>
      <aside className="learn-feed-aside">
        {loading ? <div className="learn-card p-5 text-sm">Loading your learning...</div> : nextCourse ? <section className="learn-continue-card"><div className="learn-eyebrow">Continue Learning</div><h2 className="mt-2 font-bold">{nextCourse.title}</h2>{nextCourse.thumbnail_url && <img src={nextCourse.thumbnail_url} alt="" className="mt-3 h-24 w-full rounded-md object-cover"/>}<p className="mt-2 text-sm leading-6">{nextCourse.description || "Pick up where you left off in your learning path."}</p><div className="mt-4 flex items-center gap-3"><div className="h-2 flex-1 overflow-hidden rounded-full bg-slate-200"><div className="h-full rounded-full bg-[#19b33b]" style={{ width: `${nextCourse.progress}%` }}/></div><span className="text-xs font-bold">{nextCourse.progress}%</span></div><Link to="/learn/courses/$slug" params={{slug: nextCourse.slug}} className="learn-course-action mt-3 w-full">{nextCourse.progress >= 100 ? "View course" : nextCourse.progress ? "Continue" : "Start learning"}<ArrowRight size={14}/></Link></section> : null}
        {!loading && courses.length > 0 && <div className="learn-feed-stats"><div><span>Enrolled courses</span><strong>{courses.length}</strong></div><div><span>In progress</span><strong>{inProgress.length}</strong></div><div><span>Completed</span><strong>{completed.length}</strong></div></div>}
      </aside>
    </div>
    {loading ? null : courses.length === 0 ? <div className="learn-card learn-empty"><div className="mx-auto learn-icon-tile"><BookOpen size={20}/></div><h2 className="mt-5 text-xl font-bold">Your learning starts here</h2><p className="mx-auto mt-2 max-w-md text-sm leading-6">You don't have an active course enrolment yet. Browse the available courses or contact the learning team.</p><Link to="/learn/courses" className="learn-primary-button mt-5">Explore courses <ArrowRight size={16}/></Link></div> : <section><div className="mb-3 flex items-center justify-between"><div><div className="learn-eyebrow">Your courses</div><h2 className="mt-1 text-xl font-bold">Keep learning</h2></div><Link to="/learn/courses" className="learn-see-all">View all <ArrowRight size={14}/></Link></div><div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">{courses.slice(0,6).map((course) => <article className="learn-course-card" key={course.id}><div className="learn-course-image">{course.thumbnail_url ? <img src={course.thumbnail_url} alt=""/> : <div className="learn-course-placeholder"><Sparkles size={34}/></div>}{(course.progress >= 100 || course.enrollmentStatus === "completed") && <span className="learn-course-badge"><CheckCircle2 size={13}/> Completed</span>}</div><div className="p-5"><h3 className="line-clamp-2 text-base font-bold leading-6">{course.title}</h3><div className="mt-3 flex items-center justify-between text-xs text-slate-400"><span>{course.completedLessons}/{course.totalLessons} lectures</span><span className="font-semibold text-slate-200">{course.progress}%</span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-white/10"><div className="h-full rounded-full bg-gradient-to-r from-cyan-400 to-violet-500" style={{ width: `${course.progress}%` }}/></div><Link to="/learn/courses/$slug" params={{slug: course.slug}} className="learn-course-action mt-4">{course.progress >= 100 ? "View course" : course.progress ? "Continue" : "Start learning"}<ArrowRight size={15}/></Link></div></article>)}</div></section>}
    <div className="mt-6 learn-card p-5"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><div className="learn-eyebrow">Need help?</div><h2 className="mt-1 text-base font-bold">Questions or feedback?</h2><p className="mt-1 text-sm">Message the learning team about your course, workshop or access.</p></div><Link to="/learn/messages" className="learn-secondary-button"><MessageSquare size={15}/> Open messages</Link></div></div>
  </LearnShell>;
}
