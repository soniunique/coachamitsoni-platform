...
function LearnHome() {
  ...
  return <LearnShell>
    <SectionHeader eyebrow="Feed" title="Upcoming Workshops" description="Join upcoming live sessions, catch the latest updates and continue your learning." action={<Link to="/learn/workshops" className="learn-see-all">See all <ArrowRight size={14}/></Link>} />
    <div className="learn-feed-layout">
      ...
      <aside className="learn-feed-aside">
        {loading ? <div className="learn-card p-6 text-sm">Loading your learning...</div> : nextCourse ? <section className="learn-continue-card"><div className="learn-eyebrow">Continue learning</div><h2 className="mt-2 text-xl font-bold">{nextCourse.title}</h2><p className="mt-2 text-sm leading-6 text-slate-500">{nextCourse.description || "Continue your learning path from your latest progress."}</p><div className="mt-5 flex items-center gap-3"><div className="h-2 flex-1 overflow-hidden rounded-full bg-slate-200"><div className="h-full rounded-full bg-[#19b33b]" style={{ width: `${nextCourse.progress}%` }}/></div><span className="text-sm font-bold">{nextCourse.progress}%</span></div><Link to="/learn/courses/$slug" params={{slug: nextCourse.slug}} className="learn-course-action mt-4 w-full">{nextCourse.progress ? "Continue" : "Start learning"}<ArrowRight size={15}/></Link></section> : null}
