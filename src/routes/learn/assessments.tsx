...
function Assessments() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data: enrolments, error: enrolmentError } = await supabase.from("program_enrollments").select("program_id").eq("user_id", user.id).in("status", ["active", "completed"]);
      ...
      const { data: courseData, error: courseError } = await supabase.from("courses").select("id,slug,title,description,program_id").eq("status", "published").eq("assessment_enabled", true).in("program_id", programIds).order("created_at", { ascending: false });
      if (courseError) { setError(courseError.message); setLoading(false); return; }
      const results = await Promise.all(((courseData ?? []) as Course[]).map(async course => {
