...
update public.course_assessments set max_attempts = 3 where max_attempts is null or max_attempts <> 3;
...
as $function$
declare
  a public.course_assessments%rowtype;
  v_course_id uuid;
  v_program_id uuid;
  v_total integer := 0;
  v_done integer := 0;
...
begin
  ...
  select c.id, c.program_id into v_course_id, v_program_id from public.courses c where c.id = a.course_id and c.status = 'published' and c.assessment_enabled;
  ...
  if a.time_limit_minutes is not null and a.time_limit_minutes > 0 then v_expires := v_started + make_interval(mins => a.time_limit_minutes); end if;
