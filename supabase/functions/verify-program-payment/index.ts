...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Always verify the Razorpay signature before treating a request as an idempotent replay.
    ...
    if (order.status === "paid" && order.user_id) {
      ...
      const { data: program } = await supabase.from("programs").select("title").eq("id", order.program_id).maybeSingle();
      const emailResult = await sendProgramPaymentConfirmation(supabase, order, program?.title ?? "your program", razorpay_payment_id);
      return json({ success: true, enrolled: true, alreadyProcessed: true, program_id: order.program_id, account_created: false, account_setup_required: Boolean(guestEmail), email_sent: Boolean(emailResult.sent || emailResult.alreadySent) });
    }
    ...
    if (!userId) {
      const ensured = await ensureUser(supabase, guestEmail, guestName);
      userId = ensured.user.id;
      accountCreated = ensured.created;
      const { error: profileError } = await supabase.from("profiles").upsert({ id: userId, full_name: guestName }, { onConflict: "id" });
    ...
    if (!existingEnrollment) {
      const { error: enrollmentInsertError } = await supabase.from("program_enrollments").insert({ user_id: userId, program_id: order.program_id, status: "active", enrolled_at: now });
      if (enrollmentInsertError) return json({ error: enrollmentInsertError.message }, 500);
    } else if (existingEnrollment.status === "cancelled") {
      const { error: enrollmentUpdateError } = await supabase.from("program_enrollments").update({ status: "active", enrolled_at: now, completed_at: null }).eq("id", existingEnrollment.id);
      if (enrollmentUpdateError) return json({ error: enrollmentUpdateError.message }, 500);
    }
    const { data: program } = await supabase.from("programs").select("title").eq("id", order.program_id).maybeSingle();
    await sendProgramPaymentConfirmation(supabase, order, program?.title ?? "your program", razorpay_payment_id);
    return json({ success: true, enrolled: true, program_id: order.program_id, account_created: accountCreated, account_setup_required: Boolean(guestEmail) });
  } catch (error) {
    console.error("verify-program-payment error", error);
    return json({ error: error instanceof Error ? error.message : "Unexpected payment verification error." }, 500);
  }
});
