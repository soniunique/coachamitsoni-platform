...
Deno.serve(async(req)=>{
  ...
  const signature=req.headers.get("X-Razorpay-Signature");const eventId=req.headers.get("X-Razorpay-Event-Id");
  if(!signature||!eventId)return json({error:"Missing Razorpay webhook signature or event id."},400);
  ...
  try{
    const payload=JSON.parse(rawBody);const eventType=typeof payload?.event==="string"?payload.event:"";const orderEntity=payload?.payload?.order?.entity;const paymentEntity=payload?.payload?.payment?.entity;const providerOrderId=orderEntity?.id??paymentEntity?.order_id??null;
    ...
    if(eventInsertError){if(eventInsertError.code==="23505")return json({received:true,duplicate:true});console.error("webhook event insert error",eventInsertError);return json({error:"Unable to record webhook event."},500);}
    ...
    if(eventType==="payment.failed"){await supabase.from("program_orders").update({status:"failed"}).eq("id",order.id).in("status",["created","failed"]);await supabase.from("razorpay_webhook_events").update({status:"processed",processed_at:new Date().toISOString()}).eq("event_id",eventId);return json({received:true,processed:true});}
    if(eventType!=="order.paid"&&eventType!=="payment.captured"){await supabase.from("razorpay_webhook_events").update({status:"ignored",processed_at:new Date().toISOString()}).eq("event_id",eventId);return json({received:true,ignored:true});}
    ...
    const metadata=(order.metadata??{}) as Record<string,unknown>;const guestEmail=typeof metadata.guest_email==="string"?metadata.guest_email.trim().toLowerCase():"";const guestName=typeof metadata.guest_name==="string"?metadata.guest_name.trim():"Customer";
    let userId=order.user_id as string|null;
    if(!userId){if(!guestEmail)throw new Error("Payment order is missing customer email.");const ensured=await ensureUser(supabase,guestEmail,guestName);userId=ensured.user.id;await supabase.from("profiles").upsert({id:userId,full_name:guestName},{onConflict:"id"});}
    const now=new Date().toISOString();
    const {error:paidError}=await supabase.from("program_orders").update({user_id:userId,status:"paid",provider_payment_id:paymentId,paid_at:now}).eq("id",order.id).neq("status","refunded");
    if(paidError){await supabase.from("razorpay_webhook_events").update({status:"failed",error_message:paidError.message}).eq("event_id",eventId);return json({error:"Unable to mark payment as paid."},500);}
    ...
    if(enrollmentLookupError){await supabase.from("razorpay_webhook_events").update({status:"failed",error_message:enrollmentLookupError.message}).eq("event_id",eventId);return json({error:"Unable to check program enrollment."},500);}
    if(!existingEnrollment){const {error:enrollmentInsertError}=await supabase.from("program_enrollments").insert({user_id:userId,program_id:order.program_id,status:"active",enrolled_at:now});if(enrollmentInsertError){await supabase.from("razorpay_webhook_events").update({status:"failed",error_message:enrollmentInsertError.message}).eq("event_id",eventId);return json({error:"Payment recorded, but enrollment could not be created yet."},500);}}else if(existingEnrollment.status==="cancelled"){const {error:enrollmentUpdateError}=await supabase.from("program_enrollments").update({status:"active",enrolled_at:now,completed_at:null}).eq("id",existingEnrollment.id);if(enrollmentUpdateError){await supabase.from("razorpay_webhook_events").update({status:"failed",error_message:enrollmentUpdateError.message}).eq("event_id",eventId);return json({error:"Payment recorded, but enrollment could not be restored yet."},500);}}
    await supabase.from("razorpay_webhook_events").update({status:"processed",processed_at:now}).eq("event_id",eventId);return json({received:true,processed:true,program_id:order.program_id});
