...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    const{program_id,email,full_name,mobile,mobile_number}=body;
    const guestEmail=typeof email==="string"?email.trim().toLowerCase():"";
    const guestName=typeof full_name==="string"?full_name.trim():"";
    const rawMobile=typeof mobile==="string"?mobile:(typeof mobile_number==="string"?mobile_number:"");
    const guestMobile=rawMobile.replace(/\D/g,"");
    ...
    if(!mobilePattern.test(guestMobile))return json({error:"Enter a valid 10-digit Indian mobile number."},400);

    const supabaseUrl=Deno.env.get("SUPABASE_URL")!;
    const serviceRoleKey=Deno.env.get("SUPABASE_SERVICE_ROLE_KEY");
    if(!serviceRoleKey)return json({error:"Payment service is not configured."},500);
    ...
    const razorpayResponse=await fetch("https://api.razorpay.com/v1/orders",{method:"POST",headers:{Authorization:`Basic ${basic}`,"Content-Type":"application/json"},body:JSON.stringify({amount:Math.round(priceInr*100),currency:"INR",receipt,notes:{program_id,guest_email:guestEmail,guest_name:guestName,guest_mobile:guestMobile}})});
    const razorpayOrder=await razorpayResponse.json();
    if(!razorpayResponse.ok)return json({error:razorpayOrder?.error?.description??"Unable to create payment order."},502);

    const{error:orderInsertError}=await supabase.from("program_orders").insert({user_id:authenticatedUserId,program_id,amount_inr:priceInr,currency:"INR",status:"created",provider:"razorpay",provider_order_id:razorpayOrder.id,metadata:{receipt,guest_email:guestEmail,guest_name:guestName,guest_mobile:guestMobile}});
    if(orderInsertError)return json({error:orderInsertError.message},500);

    return json({key_id:keyId,order_id:razorpayOrder.id,amount:razorpayOrder.amount,currency:razorpayOrder.currency,program_id,program_title:program.title,guest:!authenticatedUserId});
  }catch(error){
